...
var CB = (function () {
  ...
  var ROSTER = [
    ...
    { id:'spearman', size:'s', name:'槍兵', en:'Spearman', cost:3, role:'melee', elem:'steel', line:'人', up:'spearmaster', tier:1,
      hpT:5, atkT:3, spd:4,
  ...
  function killUnit(st, tgt, src) {
    ...
    // 凱歌
    if (src && hasP(src, 'triumph') && src.alive) {
      if (!src.flags.triumphUsed) {
        src.flags.triumphUsed = true;
        src.flags.extraTurn = true;
        push(st, { type: 'passive', uid: src.uid, name: '凱歌', text: 'もう一度行動する', kind: 'good' });
        logMsg(st, src.def.name + ' の【凱歌】！ 続けてもう一度行動する', 'good');
      } else {
        /* 1ラウンド1回。2体目を倒しても続かない理由が分からないという声があったので、
           使い切っていることをその場で伝える */
        logMsg(st, src.def.name + ' の【凱歌】は、このラウンドではもう使えない');
      }
    }
  }
  ...
  function performAction(st, u, actionKey, target) {
    ...
    victims.forEach(function (v) {
      /* v108: 乱れ撃ちの撃ち直し。
         乱れ撃ちは撃つ前に全弾の狙いを決めるので、途中で倒した相手に
         残りの弾が吸い込まれ、そのぶんが不発になっていた（メテオで確認）。
         狙った相手がもう倒れていたら、生きている敵へ撃ち直す。
         範囲技はこの限りではない（先に倒れた相手には当たらないのが正しい） */
      if (!v.u.alive && a.range === 'random') {
        var rest = aliveUnits(st, foe);
        if (rest.length) v = { u: rest[Math.floor(st.rnd() * rest.length)], mul: v.mul };
      }
      if (!v.u.alive) return;
      var amt = base * v.mul;
  ...
  var TEASERS = [
    ...
    { id:'spearmaster', name:'槍術士', en:'Spearmaster', line:'人', tier:2, base:'spearman', up:'dragonslayer',
      art:{ body:'human', head:'band', wep:'spear', deco:'crest' }, elem:'steel',
      tech:'刺し貫き', flavor:'穂先が長いほど、間合いは静かになる。' },
    { id:'dragonslayer', name:'竜殺し', en:'Dragonslayer', line:'人', tier:3, base:'spearmaster',
      art:{ body:'human', head:'dragonhelm', wep:'spear', deco:'scales' }, elem:'steel',
      tech:'屠竜', flavor:'その兜は、かつて彼を喰おうとした顎でできている。' },
