...
BINS=[('knight','paladin'),('paladinking','warfiend'),('warrior','berserker'),('spearman','shieldguard'),
      ...
      # 第2世代の残り（上の段）。1体だけの便は縦長1枚で出す
      ('ancientdragon',),('phantom','divinearcher'),('royalguard','kingshield'),
      ('necromancer','lich'),('ancient','colossus'),('skald','bragi'),
      ('phoenix','jotunn'),('odin',),
      ('shieldguard','royalguard'),('kingshield',)]
HEAD='''■ 第%02d便：%s ／ %s
...
'''
HEAD1='''■ 第%02d便：%s（1体だけ）
共通指定に従います。前の便で描いた絵は参考にしません。この1体の個別指定だけを見てください。
・縦長 1024×1408 の1枚。1体だけを大きく描く
・背景は画面全体をマゼンタ #FF00FF の平らな一色。縁の光なし。地面なし
・最初に添付した3枚（吟遊詩人・大賢者・闇の刃）と同じ画風・同じ塗りの厚み。頭身は個別指定に従う
・体の向きは、顔・肩・足の「見えるもの」で確認すること（共通指定の■体の向き）
'''
...
def bin_text(n, ids):
    ...
    head = HEAD1%(n,names[0]) if len(ids)==1 else HEAD%(n,names[0],names[1])
    return head+EXTRA.get(n,'')+'\n'+'\n\n'.join(block(c,i+1) for i,c in enumerate(ids))+'\n'
def main():
