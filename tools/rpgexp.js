...
.col{flex:1 1 auto;display:flex;flex-direction:column;min-height:0}
...
.mapwrap{flex:0 0 42svh;position:relative;overflow:hidden;background:#7f7866;
  border-bottom:2px solid #241f14;touch-action:none;user-select:none;-webkit-user-select:none}
...
.mapx{position:absolute;top:6px;right:6px;z-index:3;width:34px;height:34px;border-radius:50%;
  border:0;background:rgba(22,17,9,.66);color:#f6efd9;font-size:15px;cursor:pointer}
.mapctl2{position:absolute;right:6px;bottom:8px;z-index:3;display:flex;flex-direction:column;gap:6px}
.mapctl2 button{width:38px;height:38px;border-radius:10px;border:0;font:inherit;font-size:13px;
  background:rgba(22,17,9,.66);color:#f6efd9;cursor:pointer}
.pad{position:absolute;right:12px;bottom:12px;z-index:5;display:grid;
  grid-template-columns:repeat(3,52px);grid-template-rows:repeat(3,52px);gap:5px;
  opacity:1;transition:opacity .25s}
.pad.hidden{opacity:0;pointer-events:none}
...
.sheet{position:absolute;left:8px;right:8px;bottom:8px;max-height:58%;z-index:6;
...
.narr{flex:1 1 0;overflow-y:auto;min-height:7.5em;padding:10px 12px 110px;
...
.pm{background:var(--panel);border:1px solid var(--line);border-radius:8px;
  padding:7px 4px 8px;text-align:center;min-width:0}
.pm .n{font-size:10.5px;color:var(--ink2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pm .h{font-size:12.5px;color:var(--ink);font-variant-numeric:tabular-nums;margin-top:1px}
.pm .h b{font-weight:700}
.pm .bar{height:5px;background:#1a2338;border-radius:3px;margin:5px 3px 0;overflow:hidden}
.pm .bar i{display:block;height:100%;border-radius:3px}
.pm.back{opacity:.78}
.pm.back .n:before{content:"後 ";color:var(--ink3)}
body.land .wrap{flex-direction:row}
body.land .mapwrap{flex:0 0 46%;height:100svh;border-bottom:0;border-right:2px solid #241f14}
body.land .col{flex:1 1 auto}
...
function camInit(){
  ...
  el.addEventListener('pointerdown', function(ev){
    if (/^(mz_|mapx)/.test(ev.target.id || '')) return;
  ...
  document.getElementById('mz_in').onclick = function(){
    if (!S) return; if (S.cam.fit) S.cam.fit = false;
    S.cam.z = Math.min(2.6, S.cam.z * 1.3); camApply();
  };
  document.getElementById('mz_out').onclick = function(){
    if (!S) return; if (S.cam.fit) S.cam.fit = false;
    S.cam.z = Math.max(0.4, S.cam.z / 1.3); camApply();
  };
  document.getElementById('mz_fit').onclick = function(){
    if (!S) return;
    S.cam.fit = !S.cam.fit; S.cam.panx = 0; S.cam.pany = 0; camApply();
  };
...
function bossFight(){
  ...
  setTimeout(function(){
    ...
    say('（主との本式の戦闘は、次の段で入ります。いまは幕だけ下ろしています）', 'sys');
  }, 5000);
  setTimeout(function(){
...
function exitDungeon(){
  ...
  setTimeout(function(){
    try { parent.RPGDONE && parent.RPGDONE({ id: RPGQ.id, cleared: !!S.qdone,
      coin: (S.qdone ? (RPGQ.coin || 0) : 0) + (S.loot || 0) * 10,
      loot: S.loot || 0, kills: S.kills, hurt: S.hurt, steps: S.steps }); } catch (e) {}
  }, 3400);
}
...
function render(){
  ...
  document.getElementById('party').innerHTML = PARTY.map(function(p){
    ...
    var col = pct <= 30 ? 'var(--ng)' : pct <= 60 ? 'var(--warn)' : 'var(--ok)';
    return '<div class="pm'+(p.row?' back':'')+'">'
      + '<div class="n">'+p.n+'</div>'
      + '<div class="h"><b>'+p.hp+'</b>/'+p.mx+'</div>'
  ...
  Object.keys(pads).forEach(function(id){
    ...
    if (e){
      ...
      if (shut && S.been[e.via.id]) cls = 'shut';
      else if (S.been[e.far.id]) cls = 'back';
      else cls = 'unex';
    }
    b.className = 'pd ' + cls;
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  camInit();
  var narrEl = document.getElementById('narr'), padEl = document.getElementById('pad');
  var padT = null;
  var padHide = function(){
    padEl.classList.add('hidden');
    clearTimeout(padT);
    padT = setTimeout(function(){ padEl.classList.remove('hidden'); }, 750);
  };
  /* 自動スクロール（新しい文の追記）では隠さない。指とホイールのときだけ */
  narrEl.addEventListener('wheel', padHide, { passive: true });
  narrEl.addEventListener('touchmove', padHide, { passive: true });
...
<div class="wrap">
  <div class="mapwrap" id="mapwrap">
    ...
    <button class="mapx" id="mapx" title="中断">✕</button>
    <div class="mapctl2">
      <button id="mz_in">＋</button>
      <button id="mz_out">－</button>
      <button id="mz_fit">全体</button>
    </div>
  </div>
  <div class="col">
    <div class="side">
