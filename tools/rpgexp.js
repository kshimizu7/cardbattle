...
/* ================= 状態 ================= */
var PARTY = [
  { n:'聖騎士',   id:'paladin',    hp:26, mx:26, row:0 },
  { n:'狂戦士',   id:'berserker',  hp:26, mx:26, row:0 },
  { n:'槍兵',     id:'spearman',   hp:21, mx:21, row:0 },
  { n:'高僧',     id:'highpriest', hp:26, mx:26, row:1 },
  { n:'魔法使い', id:'mage',       hp:11, mx:11, row:1 },
  { n:'弓兵',     id:'archer',     hp:11, mx:11, row:1 }
];
...
function syncPartyStats(){
  try {
    ...
    PARTY.forEach(function(p){
      var d = st[p.id];
      if (d && d.hp){ p.mx = d.hp; p.hp = d.hp; if (d.name) p.n = d.name; }
    });
  } catch (e) {}
}
var S = null;
...
function camInit(){
  ...
  el.addEventListener('pointerdown', function(ev){
    if (ev.target.closest && ev.target.closest('.pad, .sheet, .mapui')) return;
  ...
  var down0 = null;
  el.addEventListener('pointerdown', function(ev){
    if (ev.target.closest && ev.target.closest('.pad, .sheet, .mapui')) { down0 = null; return; }
...
function partyTeam(){
  return PARTY.map(function(p, i){
    return { id: p.id, row: p.row, col: SLOTS6[i][1], hp: p.hp };
  });
}
...
function battleSpoils(kind){
  ...
  var NAME = { salve: '傷薬', coin: '古い硬貨のひと握り', oil: '使いさしの油' };
  say('{——骸のそばに、布に包まれたものが落ちていた|'
    + '倒れたものの傍らに、持ち物が散っている|'
    + '$who が、こぼれ落ちたものを拾い上げた}。', 'em');
  drops.forEach(function(d){
    var res = gainItem(d[0], d[1]);
    if (!res.got){ say('── ' + NAME[d[0]] + 'は、もう荷に入らない', 'sys'); return; }
    var have = d[0] === 'coin' ? S.loot : S.bag[d[0]];
    say('── ' + NAME[d[0]] + (res.got > 1 ? ' ×' + res.got : '') + ' を手に入れた（' + have + '）', 'sys');
  });
}
...
function wipeOut(){
  ...
  setTimeout(function(){
    try { parent.RPGDONE && parent.RPGDONE({ id: RPGQ ? RPGQ.id : null, cleared: false,
      wiped: true, coin: 0, loot: 0, bag: { salve: 0 }, chests: S.chests,
...
function exitDungeon(){
  ...
  setTimeout(function(){
    ...
    try { parent.RPGDONE && parent.RPGDONE({ id: RPGQ.id, cleared: !!S.qdone,
      ...
      bag: { salve: S.bag.salve || 0, oil: S.bag.oil || 0 },
...
function render(){
  ...
  if (S.bag.salve > 0) addSkill('傷薬をつかう　×' + S.bag.salve,
    hurtOne ? hurtOne.n + ' の傷が塞がる' : '傷を負った者がいない', hurtOne ? 'act' : '', function(){
      ...
      S.bag.salve--;
      hurtOne.hp = Math.min(hurtOne.mx, hurtOne.hp + 8);
  ...
  if (!S.bag.salve && !S.bag.oil && !S.loot && !dItems.length)
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  document.getElementById('mz_full').onclick = function(){
    document.body.classList.toggle('mapbig');
