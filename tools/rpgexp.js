...
/* ================= 地図の絵 =================
   Roll20 風。踏破した所は完全な形と方眼、光が届いただけの所は
   床の断片が闇に溶けて途切れる。ふだんは現在地まわりの窓表示、
   タップで全体表示（＋／－で拡大縮小）。                          */
var CELL = 40;
...
function nodeFull(n){
  ...
  else total = n.cells.length;
  return Object.keys(lit).length >= total;
}
function drawMap(){
  var ns = S.map.nodes;
  ...
  ns.forEach(function(n){
    ...
    if (nodeFull(n)){
      ...
      var lit = S.litCells[n.id] || {};
      for (var k in lit){ var xy = k.split(','); eat(+xy[0], +xy[1]); }
    }
  });
  if (!isFinite(f.x0)){ var e0 = S.map.byId[S.at]; eat(Math.floor(e0.gx), Math.floor(e0.gy)); }
  ...
  s += '<defs>'
    ...
    + '<stop offset="0" stop-color="#332d1d" stop-opacity="0.95"/>'
    + '<stop offset="1" stop-color="#332d1d" stop-opacity="0"/></linearGradient>'
    + '<linearGradient id="fS" x1="0" y1="1" x2="0" y2="0">'
    + '<stop offset="0" stop-color="#332d1d" stop-opacity="0.95"/>'
    + '<stop offset="1" stop-color="#332d1d" stop-opacity="0"/></linearGradient>'
    + '<linearGradient id="fW" x1="0" y1="0" x2="1" y2="0">'
    + '<stop offset="0" stop-color="#332d1d" stop-opacity="0.95"/>'
    + '<stop offset="1" stop-color="#332d1d" stop-opacity="0"/></linearGradient>'
    + '<linearGradient id="fE" x1="1" y1="0" x2="0" y2="0">'
    + '<stop offset="0" stop-color="#332d1d" stop-opacity="0.95"/>'
    + '<stop offset="1" stop-color="#332d1d" stop-opacity="0"/></linearGradient>'
    + '</defs>';
  ...
  ns.forEach(function(n){
    ...
    if (nodeFull(n)){
      ...
      else n.cells.forEach(function(c){ litAll[c[0] + ',' + c[1]] = 1; });
    } else {
      var lit = S.litCells[n.id] || {};
      for (var k in lit) litAll[k] = 1;
    }
  });
  var idx = cellIndex();
  ...
  ns.forEach(function(n){
    ...
    for (var k in lit){
      ...
      dirs.forEach(function(dd){
        var nk = (cxx + dd[0]) + ',' + (cyy + dd[1]);
        if (idx[nk] && !litAll[nk]){
          s += '<rect x="' + X + '" y="' + Y + '" width="' + CELL + '" height="' + CELL + '" fill="url(#' + dd[2] + ')"/>';
        }
      });
    }
  });
  ...
  /* 印は出さない（説明文に任せる）。関門の閂も、実際に前に立ってから */
  ns.forEach(function(n){
    if (!S.seen[n.id]) return;
    if (n.kind === 'corridor'){
    ...
    var icon = '', ic = '';
    if (n.goal && been){ icon = '奥'; ic = '#8a6a1a'; }
