...
var FOES = {
  ...
  pit:    { small: ['ogre','golem','harpy'],         big: ['troll','golem','ogre'] }
};
...
function wipeOut(){
  ...
  setTimeout(function(){
    try { parent.RPGDONE && parent.RPGDONE({ id: RPGQ ? RPGQ.id : null, cleared: false,
      wiped: true, coin: 0, loot: 0, bag: { salve: 0 }, gear: { eq:{}, bag:[] },
      party: partyHome(true), chests: S.chests,
...
function exitDungeon(){
  ...
  setTimeout(function(){
    ...
    try { parent.RPGDONE && parent.RPGDONE({ id: RPGQ.id, cleared: !!S.qdone,
      ...
      bag: { salve: S.bag.salve || 0, oil: S.bag.oil || 0 }, gear: gearHome(), party: partyHome(),
...
/* 傷の持ち帰り。全滅して運ばれたときは、みな虫の息のまま街に着く */
function partyHome(wiped){
  var out = {};
  PARTY.forEach(function(p){ out[p.id] = wiped ? 1 : Math.max(1, p.hp); });
  return out;
}
...
function begin(){
  ...
  PARTY.forEach(function(p){
    applyGear(p);
    /* 街で癒していない傷は、そのまま潜ることになる */
    p.hp = (carryHp && carryHp[p.id] != null) ? Math.max(1, Math.min(p.mx, carryHp[p.id])) : p.mx;
  });
