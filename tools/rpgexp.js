...
.pad{position:absolute;right:-4px;bottom:0;z-index:5;width:190px;height:186px;
...
.pd .ar{fill:#6f6a58}
.pd.unex .ar{fill:#f2c65c;filter:drop-shadow(0 0 3px rgba(242,198,92,.55))}
...
.pd.k .fl{fill:#6f6a58}
.pd.k.has .fl{fill:#f2c65c;filter:drop-shadow(0 0 4px rgba(242,198,92,.5))}
...
function camInit(){
  ...
  el.addEventListener('pointerdown', function(ev){
    if (ev.target.closest && ev.target.closest('.pad, .sheet, .mapx')) return;
...
function render(){
  ...
  /* 入口の前：最下段の一列（真ん中に大きく「入る」） */
  var eb = document.getElementById('entrybar');
  eb.classList.toggle('on', !!S.pre);
  if (S.pre){
    eb.innerHTML = '';
    var mk2 = function(label, cls, fn){
      var b = document.createElement('button');
      if (cls) b.className = cls;
      b.textContent = label; b.onclick = fn; eb.appendChild(b);
    };
    mk2('装備を整える', '', function(){
      say('（装備は Phase 2 で。いまは身ひとつでもぐる）', 'sys'); });
    mk2('入 る', 'main', function(){ S.pre = 0; enter(S.map.start); });
    mk2(RPGQ ? '戻る' : '選び直す', '', function(){
...
<div class="wrap">
  ...
  <div class="mapwrap" id="mapwrap">
    ...
    <div class="pad" id="pad">
      <svg viewBox="0 0 200 196" aria-label="操作盤">
        ...
        <g id="pd_k" class="pd k" transform="translate(170,22)">
